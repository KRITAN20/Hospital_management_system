...
export default function SignUp() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (res.data.error) {
        setLoader(false);
        setAlertMsg(res.data.errorMsg || "An error occurred!");
        setAlert(true);
        console.error(res.data.errorMsg);  // Log the error in the console
      } else {
        setLoader(false);
        setUser("Patient");
        setDepartment("");
      ...
      setAlert(true);
      console.error(error.response?.data || error);  // Log the detailed error message
    }
  };
